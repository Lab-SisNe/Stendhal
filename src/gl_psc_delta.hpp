...
#ifndef STENDHAL_GL_PSC_DELTA_HPP
#define STENDHAL_GL_PSC_DELTA_HPP
...
struct stendhal::connection:
  structure containing pointer of the target neuron (gl_psc_delta class), synaptic weight and synaptic delay.
  members:
    class gl_psc_delta * target;
    double weight;
    double delay;

class stendhal::gl_psc_delta:
  class containing all the parameters, and functions of the GL neuron with
    delta shape post synaptic current.
  ...
  private members:
    ...
    buffer variables:
      ...
      unsigned int *pbuff_pos, *pbuff_size;
        ...
        this behaviour will depend on how gl_psc_delta class is constructed, and
	...
	*target: pointer to post-synaptic neuron
        ...
        pos: value of synaptic delay in time steps (int)
  ...
  public members:
    constructors:
      stendhal::gl_psc_delta::gl_psc_delta(seed =55);
      ...
      stendhal::gl_psc_delta::gl_psc_delta(seed, *pbuff_pos, *pbuff_size);
        constructor whith global pbuff_pos and pbuff_size,
        but local random number generators.
        Note that if nodes are created with the same seed, they will all behave
        with the same sequence of random numbers. That is, if they receive all the
        exact same input, they will all behave the same.
      stendhal::gl_psc_delta::gl_psc_delta(delta_t, *prng, *pudist, *pbuff_pos, *pbuff_size);
    ...
    public functions:
      ...
      void stendhal::gl_psc_delta::resize_buffer(unsinged int len);
        resize buffer (W_exc and W_inh) size and initialize extended elements to 0.0.
      void stendhal::gl_psc_delta::connect(stendhal::gl_psc_delta *node, double w, double d);
        append *node, w and d to the post-synaptic connection list (conn_list).
      void stendhal::gl_psc_delta::add_input(double w, unsinged int d)
      ...
      void stendhal::gl_psc_delta::add_DC_input(double val);
        add val to I_ext
      double stendhal::gl_psc_delta::evaluate();
      ...
      unsingned int stendhal::gl_psc_delta::get_id();
        return the value of node_ID.
      unsigned int stendhal::gl_psc_delta::get_nodecount();
        return the value of nodecount; all nodes must have the same value.
      std::vector<struct connection>* stendhal::gl_psc_delta::get_connection();
...
namespace stendhal
{
  ...
  public:
    ...
    gl_psc_delta(double =0.1, xoroshiro128plus* =NULL, std::uniform_real_distribution<>* =NULL, unsigned int* =NULL, unsigned int* =NULL);
    ...
    gl_psc_delta(unsigned int =55, unsigned int* =NULL, unsigned int* =NULL);
    ...
    void connect(class gl_psc_delta*, double w, double d, int pos);
...
#endif // STENDHAL_GL_PSC_DELTA_HPP
